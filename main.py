...
cap = cv2.VideoCapture('./videos/input_feed.mp4')
...
while True:
    ...
    for i,box in enumerate(boxes):
        ...
        cv2.putText(frame,f'TRACKING {(x+w)/2} {(y+h)/2}',(x+10,y-3),cv2.FONT_HERSHEY_PLAIN,.7,(255,255,0),1)

    cv2.imshow('salman tracker', frame)
    k = cv2.waitKey(30)

    if k == ord("s"):
        # select the bounding box of the object we want to track (make
        # sure you press ENTER or SPACE after selecting the ROI)
        roi = cv2.selectROI("salman tracker", frame, fromCenter=False,
                            showCrosshair=True)
